...
- Action: document that the own-damage trend is the product of the two knobs, or rebase the own-damage component (for example apply claims inflation only to third party, or express own damage in start-year sum insured terms).
...
- Where: `internal/domain/triangle/triangle.go:87-89` (`dev >= devs` clamps to `devs-1`); tooltip mislabel at `internal/infrastructure/web/static/app.js:360`.
- Generated payments and especially post-close recoveries (subrogation has a long median lag) landing in development year 11+ are added to column 10, whereas Schedule P triangles never observe post-age-10 development. This biases the generated age 9-10 factor inside the realism comparison itself, and the UI tooltip labels the last column "cumulative to dev year 10" when it actually includes all later development.
- Action: drop transactions with `dev >= devs` when building the realism triangles (matching Schedule P censoring), keep the fold-in only where a lifetime-total view is wanted, and label the last display column "dev 10+".
...
- Action: document it, or let the adequacy bias decay gradually over the claim's life instead of vanishing at the first revision.

### SL-8 (medium) - the generated loss ratio's validity flag is discarded

- Where: `internal/domain/triangle/compare.go:132` (`value, _ := lossRatio(...)`).
- If generated earned premium is zero, `lossRatio` returns `(0, false)` but the 0 is scored against the band as if real - and because the observed band min is about 0 (SL-1), it can pass.
- Action: propagate `ok` and mark the check failed (or return an error) when the generated dataset has no premium.

### SL-9 (low) - the inflation index is one year too short for tail occurrences

- Where: `internal/domain/claim/inflation.go:37-49`, `internal/application/generate.go:49`, `internal/domain/claim/claim.go:100`.
- Policies written late in the final underwriting year produce occurrences in `startYear+years`, which `For` clamps to the last simulated factor, understating inflation for those claims by up to one year's drift.
- Action: build the index with `years+1` at the composition root; keep clamping as a defensive path only.

### SL-10 (low) - degenerate Dirichlet weights can produce NaN money

- Where: `internal/domain/transaction/runoff.go:172-186`, `internal/domain/shared/money.go:12-24`.
- With a very small `runoff.concentration` (validated only as positive), all `Gamma(Concentration, 1)` draws can underflow to 0, making `total == 0` and `pool*w/total` NaN; `Money(math.Round(NaN))` is an undefined int64 conversion.
- Action: guard `total <= 0` (fall back to settling at close) and reject NaN/Inf in `FromDollars`/`MulFloat`.

### SL-11 (low) - `BaseFrequency` doc says "reported claims per policy-year" but sub-excess claims are discarded after the Poisson draw

- Where: `internal/domain/lob/lob.go:42-44`, `internal/domain/claim/claim.go:107-110`.
- With a nonzero excess the realized reported frequency is materially below `BaseFrequency` (tests sidestep this with excess 0). A user calibrating `base_frequency` to an observed reported frequency gets a thinner book.
- Action: fix the doc comment to "ground-up occurrence frequency", or rescale.

### SL-12 (low) - nil claims draw their severity and probability independently of size

- Where: `internal/domain/claim/claim.go:114` and `internal/domain/claim/claim.go:121`.
- Nil claims draw their initial estimate from the same severity distribution as paying claims, and the nil probability is independent of claim size. Real withdrawn or nil claims skew small.
- Action: a future realism knob, not a bug; note as a known simplification.
...
### SL-15 (low) - the severity histogram silently drops nil claims (merged from the earlier review)

- Where: `internal/application/histogram.go:93-99` (each claim's total paid feeds the severity histogram), `internal/application/histogram.go:46-54` (`LogHistogram` ignores non-positive values).
- Nil claims (paid 0) vanish from the severity chart with no indication - 8% of claims at the default preset's `nil_probability: 0.08`.
- Action: add a UI caption ("paying claims only") or an explicit zero bucket.
...
### MF-3 (medium) - "new short-tail classes are added by writing a new YAML file, no code changes" is only true for the CLI path

- Where: `README.md:69` vs `internal/infrastructure/config/config.go:141-148` (UI presets require code registration), `cmd/claimsgen/main.go:116` (UI realism always loads the private passenger auto reference dirs), `internal/domain/lob/lob.go:264-280` and `:302-318` (recovery and severity sub-blocks must validate even when their probability or weight is 0).
- The roadmap (`docs/roadmap.md:24-27`) states this honestly; the README overstates it, and a new-class YAML author hits validation friction on features they turned off.
- Action: soften the README sentence to match the roadmap, and relax validation of sub-blocks whose probability or weight is 0.

### MF-4 (medium) - no seasonality, no catastrophe or event clustering, claims independent across policies

- Where: `internal/domain/claim/claim.go:100` (occurrence uniform over the cover period), `:70-79` (independent Poisson per policy; the only cross-policy dependence is the shared inflation path).
- Reserving demos often want an accident-year shock or a seasonal quarter to stress methods; a diagnostic actuary will notice frequency is flat within every year.
- Action: note as a known simplification; a catastrophe-event or seasonality knob is a natural roadmap item.

### MF-5 (medium) - no LICENSE or CONTRIBUTING file despite plans to open to the community - addressed 2026-07-18

- Where: repo root (verified absent at review time); `docs/mission.md:46` and `docs/roadmap.md:29` both plan to open the tool to the wider actuarial community.
- Action: add a LICENSE before any community opening; a CONTRIBUTING note can wait until then.
- Resolution: an MIT LICENSE (copyright Stephan Marais) was added at the repo root. A CONTRIBUTING note remains deferred until the community opening.

### MF-6 (low) - each year's book is an independent cohort; no policy renews

- Where: `internal/domain/policy/book.go:38-58` - only the count recurses year to year; every policy is a fresh ID with one 12-month term.
- Real exposure data shows renewal chains and persistent policyholder risk; per-policy claim histories cannot correlate across years. Acceptable for aggregate reserving demos, but worth stating given the mission's "real claims system extract" framing.
- Action: document the simplification; consider renewals (persistent risk factors) when a second class lands.
...
### MF-8 (low) - the insurer prices risk perfectly (merged from the earlier review)

- Where: `internal/domain/policy/book.go:79` (premium = sum insured x rate x the exact risk factor), `internal/domain/claim/claim.go:74` (the same factor drives claim frequency).
- The simulated insurer has no pricing error, so loss ratios are more stable across cohorts than real books (MF-1's calendar-year drift aside). Fine for reserving demos, the stated mission; worth a parameter (partial risk-factor credibility in the premium) if the tool is ever used for pricing-adjacent demos.
...
### R-3 (medium) - validation accepts NaN for every float parameter

- Where: `internal/domain/lob/lob.go:185-363` (surfaced through `config.Load` and the JSON API).
- All range checks are comparisons, and every comparison with NaN is false, so `growth_factor: .nan` (valid YAML) passes validation and produces garbage or panicking distuv draws instead of a clear error.
- Action: add a NaN/Inf sweep over the float fields in the validators.
...
### R-5 (low) - `writeJSON` ignores encoding errors, so a failed encode yields a truncated 200 response

- Where: `internal/infrastructure/web/server.go:134-138`; `Band{Min: +Inf}` is reachable via `internal/domain/triangle/compare.go:119` when no reference has usable premium.
- `encoding/json` refuses NaN and ±Inf; the client then receives status 200 with an empty body and `renderResults(null)` throws.
- Action: handle the encode error and sanitize ±Inf in `realismView` the way `triangleView` already sanitizes NaN. Related: `CompareToReference` with an empty refs slice returns a silently failing report rather than an error.

### R-6 (low) - JSON request decoding is lax while YAML is strict

- Where: `internal/infrastructure/web/server.go:109` vs `internal/infrastructure/config/config.go:111-119`.
- The JSON decoder does not call `DisallowUnknownFields`, so a misspelled key is silently dropped and defaults to zero, unlike the same typo in YAML.
- Action: set `DisallowUnknownFields` to match the config layer.

### R-7 (low) - seed values above 2^53 are silently rounded by the UI

- Where: `internal/infrastructure/web/static/app.js:186`, `internal/infrastructure/web/server.go:98`.
- `Number(...)` loses integer precision above 2^53 while the server field is uint64, so a 64-bit seed pasted from a CLI run reproduces different data.
- Action: send the seed as a string, or document a 2^53 limit with `max=` on the input.

### R-8 (low) - front-end error paths when the preset failed to load

- Where: `internal/infrastructure/web/static/app.js:67`, `:163-174`, `:496`.
- If `loadPreset` fails, Generate surfaces a raw TypeError banner and the "Reset to preset defaults" handler throws uncaught with no banner at all.
- Action: guard both paths on a null preset with a clear message and wrap the reset handler in the same catch-to-banner pattern.

### R-9 (low) - `ui --port 0` prints an unusable URL

- Where: `cmd/claimsgen/main.go:121-126`.
- The startup line prints the requested port, not the bound address, so an OS-assigned port prints `http://127.0.0.1:0`.
- Action: print `ln.Addr()`.

### R-10 (low) - out_dir semantics are opaque: relative paths resolve against the server's working directory

- Where: `internal/infrastructure/web/server.go:113-115`, `:127`, `internal/infrastructure/web/static/index.html:24`.
- The default `output` writes relative to wherever `claimsgen ui` was launched, and the run header echoes the raw input, so users launched from a shortcut cannot find the CSVs.
- Action: resolve to an absolute path server-side and return it in `run.out_dir`.

### R-11 (low) - CSV writer relies on fields never containing delimiters, with no escaping layer

- Where: `internal/infrastructure/csv/writer.go:21-45`.
- Rows are built with `fmt.Sprintf` and no quoting; today all fields are numeric, ISO dates, or fixed enums (verified), so output is correct, but any future free-text column breaks the format silently.
- Action: pin the invariant with a comment, or switch to `encoding/csv`.

### R-12 (low) - screenshot script hangs the full 120 s timeout on generation failure and hardcodes a Windows Chrome path

- Where: `tools/screenshots/screenshots.js:10-20`.
- `generateAndWait` waits for the banner to hide, so an error banner never satisfies the predicate; the default Chrome path is Windows-only (env override exists).
- Action: also watch for a visible banner and reject immediately with its text; derive the Chrome path per platform.
...
### RF-2 (medium) - the determinism test does not pin the "same seed = byte-identical output" claim

- Where: `internal/application/generate_test.go:62-69`.
- It compares only slice lengths for policies and claims and element equality for transactions; premiums, dates, estimates, and the nil/own-damage/reopen fields are never compared, so a nondeterminism bug that keeps counts stable would pass.
- Action: `reflect.DeepEqual` on the entire dataset, plus a golden-hash test on the CSV bytes.

### RF-3 (low) - duplicated mean-one lognormal helper with a weaker guard

- Where: `internal/domain/policy/book.go:98-105` vs `internal/domain/shared/distribution.go:5-10`.
- The private copy guards `== 0` where the shared one guards `<= 0`. Delete the book.go copy and call `shared.MeanOneLogNormal`.
...
- The nil branch repeats the sigma-decay and target computation of the main loop with a different remaining-source and a different floor rule (`floorRevisions` vs always-floor). A `remaining()` closure plus one unconditional keep-open floor removes the duplication and one boolean parameter, and eases adding episode types for long-tail lines. While there, replace the `event.kind` magic numbers 0/1 (`internal/domain/transaction/runoff.go:64-69`, `:126`, `:131`, `:186`) with named constants.

### RF-5 (low) - hardcoded term and calendar constants that should be parameters or named constants

- Where: `internal/domain/policy/book.go:75` (364-day term), `internal/domain/transaction/runoff.go:99` (365 divisor), one-cent floors at `internal/domain/claim/reopen.go:42-44`, `internal/domain/transaction/runoff.go:110-111` and `:139-141`, `internal/domain/transaction/recovery.go:107`.
- The 12-month term is baked in; commercial property or long-tail liability per the roadmap may need different terms. Promote term length to `BookParams`; name the cent-floor constant once.

### RF-6 (low) - validation error prefixes are inconsistent with the YAML paths

- Where: `internal/domain/lob/lob.go:302-338`.
- `severity.*` and `close_lag.*` errors omit the `claims.` prefix every sibling field uses, so a user grepping their YAML for the reported path will not find it. Prefix them `claims.severity.` and `claims.close_lag.`.
...
### RF-8 (low) - dead `lob_id` field in the generate request

- Where: `internal/infrastructure/web/server.go:97`, `internal/infrastructure/web/static/app.js:184`.
- The client sends `lob_id` but the handler never reads it. Drop it, or use it to load the preset server-side when `params` is omitted.
...
### RF-12 (low) - small robustness and style nits

- `internal/domain/claim/claim.go:112`: local `close` shadows the builtin; rename to `closeDate`.
- `internal/domain/claim/claim.go:46` and tests compare `shared.Date` structs with `==`; this works only because every date is built via `time.Date` in UTC. Add `IsZero()`/`Equal()` so a future constructor (for example parsing) cannot silently break it.
- `internal/infrastructure/csv/writer.go:54-72`: write errors are only observed at `Flush` and a panic in the row callback leaks the file handle; use `defer f.Close()` for the error paths.
- `internal/infrastructure/schedulep/reader.go:152-163`: `toTriangle` assumes contiguous origin years; a file missing a middle year silently misaligns `StartYear + i`. Validate contiguity and error otherwise.
- Global transactions are claim-block ordered, not date ordered (`internal/domain/transaction/recovery.go:60-70`); this is deliberate "registration order", but the CSV consumer contract should state it, since reserving tools often expect a date-sorted ledger.
- `schedulep.LoadFile` and `LoadDir` (`internal/infrastructure/schedulep/reader.go:69`, `:118`) are only used by their own tests; production code uses `LoadFS`. If they are deliberate seams for per-line-of-business reference loading, fine; otherwise trim (merged from the earlier review).
...
### D-1 (low) - stale and broken doc references

- `docs/roadmap.md:10` says "~143 Schedule P reference datasets"; the shipped count is 289 (143 dec2025 + 146 sep2011, verified on disk). `README.md:73` is correct.
- `docs/background-context.md:3` points at `docs/transcripts/`; the actual location is `docs/raw user inputs/transcripts/`.
- All five specs under `docs/superpowers/specs/` still carry "Status: approved pending review" (or "approved") though every one has shipped; a "shipped" status pass would help, since the roadmap points readers at spec statuses.

### D-2 (low) - a handful of exported symbols lack godoc

- `internal/domain/policy/book.go:30` (`NewBookSimulator`), `internal/domain/claim/claim.go:55` (`NewClaimSimulator`), `internal/domain/transaction/runoff.go:46` (`NewRunoffSimulator`), `internal/domain/transaction/recovery.go:33` (`NewRecoverySimulator`), `internal/domain/claim/reopen.go:19` (`NewReopenSimulator`), and the config DTO types (`internal/infrastructure/config/config.go:32-109`, covered only by a group comment). Everything else sampled is well documented.

### D-3 (low) - test gaps worth closing

- The realism gate runs on a single seed (42) only (`internal/application/realism_test.go:24`); a recalibration could pass on seed 42 and fail on most others. Run it on two or three seeds.
- No reader test asserts sep2011 vintage content; `TestLoadKnownCompany` pins dec2025 values only (`internal/infrastructure/schedulep/reader_test.go`). Add one known-value assertion for a sep2011 company including `StartYear == 1988`.
- `Summary.LossRatio` (gross paid over earned premium) shares a name with the realism loss ratio (net incurred over earned premium); the two UI tabs can show different loss ratios for the same run with no label explaining why. Label both ("gross LR" / "net LR vs Schedule P") or add a doc note.
...
2. SL-5 and RF-2: close the two gaps in the reproducibility contract (nil-claim knob isolation, full-dataset determinism test) before more features stack on top.
3. SL-3 and SL-4: cap own damage at sum insured and untangle the two inflation knobs - both distort severity levels and trend calibration.
4. R-1, R-2, R-3: server guardrails (mutex or 409 on concurrent runs, input caps plus context cancellation, NaN validation) before sharing the UI with the team.
